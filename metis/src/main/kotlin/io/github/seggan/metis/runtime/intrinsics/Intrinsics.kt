...
object Intrinsics {
    ...
    fun registerOneArg(name: String, fn: State.(Value) -> Value) {
...
class OneArgFunction(private val fn: State.(Value) -> Value) : CallableValue {
...
class TwoArgFunction(private val fn: State.(Value, Value) -> Value) : CallableValue {
    ...
    override fun call(nargs: Int): CallableValue.Executor = object : CallableValue.Executor {
        override fun step(state: State): StepResult {
            ...
            state.stack.push(state.fn(arg1, arg2))
            return StepResult.FINISHED
        }
    }
}