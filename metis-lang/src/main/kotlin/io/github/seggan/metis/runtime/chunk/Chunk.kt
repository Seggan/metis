        ...
        override fun call(): CallableValue.Executor = object : CallableValue.Executor {
            ...
            private fun stepImpl(state: State, insn: Insn): StepResult {
                when (insn) {
                    ...
                    is Insn.GetUpvalue -> state.stack.push(upvalues[insn.index].get())
                    is Insn.SetUpvalue -> upvalues[insn.index].set(state.stack.pop())
                    ...
                    is Insn.PushClosure -> state.pushChunk(insn.chunk)
                    ...
                    is Insn.CloseUpvalue -> {
                        val upvalue = state.openUpvalues.first { it.template === insn.upvalue }
                        upvalue.close()
                    }
