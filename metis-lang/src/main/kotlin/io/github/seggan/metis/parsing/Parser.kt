...
import io.github.seggan.metis.runtime.value.MetisBytes
import io.github.seggan.metis.runtime.value.MetisNull
import io.github.seggan.metis.runtime.value.MetisNumber
import io.github.seggan.metis.runtime.value.MetisString
...
class Parser(tokens: List<Token>, private val source: CodeSource) {
    ...
    private val stringConstantPool = mutableMapOf<String, MetisString>()
    ...
    fun parse(): AstNode.Block {
        ...
        return AstNode.Block(
            statements + AstNode.Return(AstNode.Literal(MetisNull, returnSpan), returnSpan),
            returnSpan
        )
    }
    ...
    private fun parseReturn(): AstNode.Return {
        ...
        val expr = tryParse(::parseExpression) ?: AstNode.Literal(MetisNull, startSpan)
    ...
    private fun parseString(): AstNode.Literal {
        ...
        val value = stringConstantPool.getOrPut(text) { MetisString(text) }
    ...
    private fun parseBytes(): AstNode.Literal {
        ...
        return AstNode.Literal(
            MetisBytes(token.text.substring(1, token.text.length - 1).escape().encodeToByteArray()),
            token.span
        )
    }
    ...
    private fun parseNumber(): AstNode.Literal {
        ...
        return AstNode.Literal(MetisNumber.of(token.text.toDouble()), token.span)
    }
    ...
    private fun parseFunctionDef(startSpan: Span, name: String? = null): AstNode.FunctionLiteral {
        ...
        if (block.lastOrNull() !is AstNode.Return) {
            ...
            nodes.add(AstNode.Return(AstNode.Literal(MetisNull, block.span), block.span))
    ...
    private fun parseVarDecl(): AstNode.VarDecl {
        ...
        return AstNode.VarDecl(
            visibility,
            name,
            value ?: AstNode.Literal(MetisNull, start.span + previous.span),
